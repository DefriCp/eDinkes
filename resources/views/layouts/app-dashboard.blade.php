...
<body class="bg-gray-100 text-gray-800">
  {{-- TOPBAR (kosong dulu) --}}
  <header class="bg-black text-white relative z-30"></header>
  ...
  <div class="flex relative">
    ...
    <aside class="bg-white border-r min-h-[calc(100vh-56px)] transition-all duration-200 sticky top-0 z-20"
      ...
      <div class="h-14 flex items-center justify-between px-3 border-b">
        <div class="font-bold text-lg" x-show="open" x-cloak>Dinas Kesehatan</div>

        {{-- tombol collapse/expand sidebar --}}
        <button class="ml-auto inline-flex items-center justify-center w-9 h-9 rounded hover:bg-gray-100"
                @click="open=!open"
      ...
      <nav class="px-2 py-3 space-y-1">
        ...
        <div class="px-2"
             ...
             x-effect="if(!open) openMenu=false">

          <button type="button"
                  @click="openMenu=!openMenu"
                  :aria-expanded="openMenu"
                  class="w-full flex items-center justify-between gap-3 px-3 py-2 rounded hover:bg-gray-100 text-gray-700">
            ...
            <span x-show="open" x-cloak class="transition" x-bind:class="openMenu ? 'rotate-180' : ''">
              <x-heroicon-o-chevron-down class="w-4 h-4" aria-hidden="true"/>
            </span>
          </button>

          {{-- Submenu (tanpa x-collapse) --}}
          <div x-show="openMenu"
            ...
            <a href="{{ route('tenagapyd.index') }}"
               class="flex items-center gap-2 px-3 py-2 rounded hover:bg-gray-100
                      {{ request()->routeIs('tenagapyd.*') ? 'bg-gray-100 text-gray-900' : 'text-gray-700' }}">
              <x-heroicon-o-user-group class="w-4 h-4 shrink-0" aria-hidden="true"/>
              <span>Daftar Jumlah Kader Posyandu</span>
            </a>
